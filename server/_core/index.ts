...
async function startServer() {
  ...
  // Rate limiting for API routes (basic) - 300 req/min for SPA
  app.use("/api", apiRateLimit);
  
  // Advanced rate limiting for specific sensitive endpoints
  app.use("/api/trpc/adminAuth.login", loginRateLimit);
  app.use("/api/trpc/security.passwordReset.request", passwordResetRateLimit);
  app.use("/api/trpc/security.passwordReset.reset", passwordResetRateLimit);
  app.use("/api/trpc/contacts.submit", contactFormRateLimit);
  ...
  // Stricter rate limit for OAuth endpoints - 30 req/15min
